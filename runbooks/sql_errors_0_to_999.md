# SQL Server Database Engine events and errors (0 to 999)

Source: https://learn.microsoft.com/en-us/sql/relational-databases/errors-events/database-engine-events-and-errors-0-to-999?view=sql-server-ver16
Catalog version in this runbook: SQL Server 2022 (16.x) / sql-server-ver16
Error text is from sys.messages. "Books Online" in a message means SQL Server product documentation.

## Support next steps (incident copilot)

1. Capture error number, severity, state, and the full message from the log or SSMS.
2. Look up the number in the catalog below.
3. Common 0-999 hits:
   - 102 Incorrect syntax
   - 137 Must declare the scalar variable
   - 207 Invalid column name
   - 208 Invalid object name
   - 229 / 230 / 262 permission denied
   - 515 Cannot insert NULL
   - 547 constraint conflict
   - 701 out of memory (if present in this range for this version)
4. Severity 17+ is often resource or engine; 21+ is severe — capture dump/time and escalate.
5. Event logged = Yes means it should appear in the SQL error log / Windows event log.
6. Do not paste PAN, passwords, or real customer rows into this copilot.
...
## Catalog (Error | Severity | Logged | Description)
...
360 | 16 | No | The target column list of an INSERT, UPDATE, or MERGE statement cannot contain both a sparse column and the column set that contains the sparse column. Rewrite the statement to include either the sparse column or the column set, but not both.
...
823 | 24 | Yes | The operating system returned error %ls to SQL Server during a %S_MSG at offset %#016I64x in file '%ls'. Additional messages in the SQL Server error log and operating system error log may provide more detail. This is a severe system-level error condition that threatens database integrity and must be corrected immediately. Complete a full database consistency check (DBCC CHECKDB). This error can be caused by many factors; for more information, see SQL Server Books Online.
...
825 | 10 | Yes | A read of the file '%ls' at offset %#016I64x succeeded after failing %d time(s) with error: %ls. Additional messages in the SQL Server error log and operating system error log may provide more detail. This error condition threatens database integrity and must be corrected. Complete a full database consistency check (DBCC CHECKDB). This error can be caused by many factors; for more information, see SQL Server Books Online.
...
833 | 10 | No | SQL Server has encountered %d occurrence(s) of I/O requests taking longer than %d seconds to complete on file [%ls] in database id %d. The OS file handle is 0x%p. The offset of the latest long I/O is: %#016I64x. The duration of the long I/O is: %I64u ms.
...
844 | 10 | No | Time out occurred while waiting for buffer latch -- type %d, bp %p, page %d:%d, stat %#x, database id: %d, allocation unit id: %I64d%ls, task 0x%p : %d, waittime %d seconds, flags 0x%I64x, owning task 0x%p. Continuing to wait. %ls
845 | 17 | No | Time-out occurred while waiting for buffer latch type %d for page %S_PGID, database ID %d. %ls
846 | 10 | No | A time-out occurred while waiting for buffer latch -- type %d, bp %p, page %d:%d, stat %#x, database id: %d, allocation unit Id: %I64d%ls, task 0x%p : %d, waittime %d seconds, flags 0x%I64x, owning task 0x%p. Not continuing to wait.
847 | 10 | Yes | Timeout occurred while waiting for latch: class '%ls', id %p, type %d, Task 0x%p : %d, waittime %d seconds, flags 0x%I64x, owning task 0x%p. Continuing to wait. %ls
...
855 | 10 | Yes | Uncorrectable hardware memory corruption detected. Your system may become unstable. Please check the operating system error log for more details.
...
898 | 10 | No | Buffer Pool scan took %I64d seconds: database ID %d, command '%ls', operation '%ls', scanned buffers %I64d, total iterated buffers %I64d, wait time %I64d ms. See 'https://go.microsoft.com/fwlink/?linkid=2132602' for more information.
...
Full range index: https://learn.microsoft.com/en-us/sql/relational-databases/errors-events/database-engine-events-and-errors
This slice: https://learn.microsoft.com/en-us/sql/relational-databases/errors-events/database-engine-events-and-errors-0-to-999?view=sql-server-ver16
